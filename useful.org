...
#+begin_src elisp :results silent
  ...
  (defun clj ()
    ...
    (interactive)
    (insert-src-block "clojure" "(clj" "pp :session aa"))
  ...
  (defun pyo ()
    ...
    (interactive)
    (insert-src-block "python" "(pyr" "output :session one"))
  ...
  (defun insert-org-file-header (title)
    ...
    (insert title)
    (let* ((dm1 (= ?1 (elt (cadr (butlast (reverse (split-string (buffer-file-name) "/")) 2)) 2)))
           (course-first-digit (if dm1 2 3)))
      (insert (format "
#+language: en
#+options: H:4 num:nil toc:nil \\n:nil @:t ::t |:t ^:t *:t TeX:t LaTeX:t d:(not \"HINT\")
#+startup: entitiespretty showeverything
#+setupfile: ~/.spacemacs.d/theme-bigblow.setup
#+LATEX_HEADER: \\input{\\detokenize{~/.spacemacs.d/cse%d80preamble.tex}}
#+LATEX: \\input{\\detokenize{~/.spacemacs.d/cse%d80headerbox.tex}}
" course-first-digit course-first-digit))))
  ...
  (defun commit-and-push-to-repo (repo file-to-commit commit-message)
    (let ((git-repo repo))
