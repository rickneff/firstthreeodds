...
* A Last Entry
  ...
  From now on, useful updates will be published as elisp packages, which, when
  their availability is announced, can be retrieved by the following function:
...
#+BEGIN_SRC emacs-lisp :results silent :tangle yes
  ...
  (defun retrieve-package (package-name)
    ...
    (let* ((package-directory (expand-file-name "~/.emacs.d/packages"))
           (package-server-url "https://firstthreeodds.org/packages")
           (already-retrieved-package-file (format "%s/%s.el" package-directory package-name))
           (size-of-file (or (nth 7 (file-attributes already-retrieved-package-file)) 0))
      ...
      (with-current-buffer temp-buffer
        (unwind-protect
            (let* ((headers (eww-parse-headers))
                   ...
                   (end (point-max)))
              (put 'retrieve-package :last-headers headers)
              (unless (= size-of-file content-length)
                ;; either not already retrieved, or else size has changed
                (write-region (- end content-length) end already-retrieved-package-file)
                (setq size-of-file content-length)))
          (kill-buffer temp-buffer)))
      size-of-file))
#+END_SRC

  For example, here is a useful package when in the mood to play with (small)
  primes:
#+BEGIN_SRC emacs-lisp
  (retrieve-package 'first-ten-million-primes)
#+END_SRC

  Highly recommended reading: the endnote in [[http://firstthreeodds.org/mebewiyo/book/latest][The Book]] keyed by the phrase \ldquo{}just
  use true and false\rdquo found in the very first paragraph of ABF.

#+BEGIN_SRC emacs-lisp
  (require 'first-ten-million-primes)
#+END_SRC

  You decide how best to use this resource (the \ldquo{}first-ten-million-primes\rdquo{}
  package) and feature (the =require= function).
